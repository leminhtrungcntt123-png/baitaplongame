    ...
    mWaveManager(mEnemies, mEnemy1Texture, mEnemy3Texture)
{
    ...
    mPlayer = std::make_unique<Player>(mPlayerTexture, mBulletTexture, mPlayerBullets);
...
// Setup Thông Báo "VÒNG 2"
    ...
    mWaveNotifyText.setString("VONG 2"); // (Sẽ được cập nhật)
...
void Game::processInput()
{
    ...
    while (mWindow.pollEvent(event))
    {
        ...
        {
            // (Đây là code cũ của bạn, không đổi)
            bool shotFired = mPlayer->handleInput(event, mBulletSpeed);
            if (shotFired)
            {
                mShootSound.play();
            }
...
void Game::updatePlayer(float deltaTime)
{
    ...
    mPlayer->update(deltaTime, mWindow.getSize().x);
...
void Game::render()
{
    ...
    else
    {
        ...
        else if (mWaveManager.getState() == WaveManager::State::Intermission)
        {
            ...
            std::string waveStr = "VONG " + std::to_string(mWaveManager.getNextWaveNumber());
    ...
    // 4. Hiển thị (Luôn luôn làm, ở cuối cùng)
    mWindow.display();
}