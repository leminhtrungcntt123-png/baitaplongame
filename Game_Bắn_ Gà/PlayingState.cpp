...
#include "../header/WonState.h" // <-- THÊM DÒNG NÀY

// --- HÀM DỰNG (CONSTRUCTOR) CỦA CHUYÊN GIA GAMEPLAY ---
PlayingState::PlayingState(Game *game) : BaseState(game), // 1. "Kết nối" (Connect) "Hợp đồng" (Contract)
                                         mLoseBackgroundTexture(game->mLoseBackground),
                                         mWonBackgroundTexture(game->mWonBackground),
                                         ...
                                         mPlayState(PlayState::Playing) // (Bắt đầu "ở" (at) "trạng thái" (state) "Playing" (Playing))
{
    ...
    if (mGame->mFontLoaded)
    {
        ...
        mWaveNotifyText.setString("ROUND 2"); // (Tạm)
...
void PlayingState::updateBullets(float deltaTime)
{
    ...
    for (int i = (int)mEnemyBullets.size() - 1; i >= 0; i--)
    {
        ...
        if (mEnemyBullets[i].sprite.getPosition().y > 600.f)
        { // (Sửa: Dùng 600.f)
...
void PlayingState::updateEntities(float deltaTime)
{
    for (int i = (int)mEnemies.size() - 1; i >= 0; i--)
    {
        ...
        if (mEnemies[i]->getPosition().y > 600.f)
        { // (Sửa: Dùng 600.f)
        ...
        else if (!mEnemies[i]->isAlive())
        {
            ...
            // 2. Quy tắc: Ưu tiên rơi Mega trước
            if (drop == LootType::MegaUpgrade)
            {
                mPowerups.push_back(Powerup(mGame->mItemChangePlayerTexture, deadPos, Powerup::MegaUpgrade));
            }
            // 3. Nếu đủ 15 kill rơi HP
            else if (mKillCount >= 15)
            {
            ...
            // 4. Nếu chưa đủ rơi
            else if (drop == LootType::UpgradeGun)
            {
    ...
    for (int i = (int)mPowerups.size() - 1; i >= 0; i--)
    {
        ...
        if (mPowerups[i].sprite.getPosition().y > 600.f)
        { // (Sửa: Dùng 600.f)
...
void PlayingState::checkGameState()
{
    ...
    if (mPlayState == PlayState::Playing)
    {
        ...
        if (!mPlayer->isAlive())
        {
            ...
            mGame->changeState(std::make_unique<GameOverState>(mLoseBackgroundTexture, mGame));
        }
        // 2. Thắng toàn bộ Game
        else if (mWaveManager.isAllWavesFinished())
        {
            mGame->changeState(std::make_unique<WonState>(mWonBackgroundTexture, mGame));
        }
...
void PlayingState::updateHUD()
{
    ...
    if (timeInt <= 5)
        mTimerText.setFillColor(sf::Color::Red);
    else
        mTimerText.setFillColor(sf::Color::Cyan);
...
void PlayingState::render(sf::RenderWindow &window)
{
    ...
    {
        std::string waveStr = "ROUND " + std::to_string(mWaveManager.getNextWaveNumber());
